...
public:
    int minOperations(std::string s, int k) {
        ...
        // مجموعتين لتخزين الحالات غير المزارة بناءً على الزوجية (Parity)
        ...
        while (!q.empty()) {
            ...
            // تحديد النطاق الممكن لعدد الأصفار الجديد z'
            ...
            // البحث عن الحالات غير المزارة في النطاق [min_z_prime, max_z_prime]
            ...
            // حذف الحالات المزارة من المجموعة
            for (int z : to_erase) {
                s_set.erase(z);
            }
        }
